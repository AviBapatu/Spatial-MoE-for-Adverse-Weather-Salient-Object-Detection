...
from src.diagnostics import MoEDiagnosticsEngine, ExpertSimilarityAnalyzer
...
def main():
    ...
    print(f"deep_supervision: {use_deep_supervision}")
    print(f"-------------------------\n")
    ...
    for ds_name, loader in datasets_to_run:
        print(f"Running diagnostics on {ds_name}...")
        ...
        for scale in ['moe_4', 'moe_8', 'moe_16']:
            ...
            if warnings:
                print(f"[{ds_name}] {scale} WARNINGS: {warnings}")
                all_warnings.extend([f"[{ds_name}] {scale}: {w}" for w in warnings])
                
        # Run ExpertSimilarityAnalyzer
        print(f"Running ExpertSimilarityAnalyzer on {ds_name} (using last batch)...")
    ...
    for w in all_warnings:
        print(w)
    if not all_warnings:
        print("No WARNINGS or REDUNDANT_PAIR found.")
