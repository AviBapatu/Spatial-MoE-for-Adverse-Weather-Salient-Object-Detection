    ...
    try:
        ...
        try:
            ...
            with autocast(device_type='cuda', dtype=torch.float16):
                ...
                nn.BCEWithLogitsLoss()(dummy_edge, edges)
...
def run_empty_batch_check(result_file):
    """
    Verify that a single expert handles zero-active-token input without crashing,
    and that DDP's find_unused_parameters bookkeeping sees every expert as 'used'.
    """
    try:
        from src.moe_layer import SpatialMoELayer

        layer = SpatialMoELayer(dim=256, num_experts=8, k=2)
        layer.eval()

        empty_input = torch.empty(0, 256)
        expert = layer.experts[0]

        # 1. Expert forward on empty batch — must not raise, must return [0, 256]
        out = expert(empty_input)
        assert_val(out.shape == (0, 256), f"Expected [0, 256], got {out.shape}")
        assert_val(torch.isfinite(out).all(), "Expert output contains NaN/Inf on empty batch")

        # 2. Backward through every expert on a trivial scalar to confirm grad registration
        layer.train()
        total_scalar = torch.tensor(0.0, requires_grad=False)
        for i, exp in enumerate(layer.experts):
            exp_out = exp(empty_input)
            # Sum of an empty tensor is 0.0 — backward produces zero grad but still registers it
            scalar = exp_out.sum()
            total_scalar = total_scalar + scalar
        ...
        for i, exp in enumerate(layer.experts):
            for name, p in exp.named_parameters():
                assert_val(
                    p.grad is not None,
                    f"Expert {i} param '{name}' has None grad after empty-batch backward"
                )

        if int(os.environ.get("RANK", 0)) == 0:
            with open(result_file, "w") as f:
                json.dump({"status": "PASS", "check": "empty_batch"}, f)
    except Exception as e:
        if int(os.environ.get("RANK", 0)) == 0:
            with open(result_file, "w") as f:
                json.dump({"status": "FAIL", "reason": str(e), "check": "empty_batch"}, f)
        raise e

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--mode", type=str, required=True, choices=["smoke1gpu", "ddp", "memory", "resume_a", "resume_b", "empty_batch"])
    parser.add_argument("--data_root", type=str, default="data/WXSOD_data")
    ...
    elif args.mode == "empty_batch":
        run_empty_batch_check(args.result_file)
    else:
    ...
    if (not hasattr(args, "mode") or args.mode == "smoke1gpu" or int(os.environ.get("RANK", 0)) == 0) and os.path.exists(args.result_file):
        ...
        if args.config_hash is not None:
            data["config_hash"] = args.config_hash
