...
Code, configurations and the per-run evaluation artifacts are available at \url{https://github.com/AviBapatu/Spatial-MoE-for-Adverse-Weather-Salient-Object-Detection}.
...
\subsection{Setup}
We evaluate on WXSOD~\cite{wxsod2025} (\url{https://github.com/C-water/WXSOD}), which provides 12,891 training images (scene-aware 80/20 split), 1,500 synthetic test images across 9 weather categories, and 554 real-world test images across 5 categories (fog, rain, snow, dark, low-light).
...
are the weakest component: boundary F1 is 0.3601 on real test, so while the
saliency masks are accurate, fine boundary detail is not.
...
Fourth, routing entropy sits at 99.8--100\% of the ceiling over the eight experts, so the decoder's entropy channel is nearly constant; its contribution has not been isolated by an ablation.
